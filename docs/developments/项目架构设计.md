...
| `chat_ws.py` | 文本聊天、实时语音、VAD、ASR 自动提交、TTS 分段事件 |
...
前端 `WebSocketManager` 负责把服务端消息类型分发为前端事件，例如 `chat:chunk`、`chat:complete`、`chat:interrupted`、`asr:transcript`、`vad:listen-state`、`vad:interrupt`。如果新增 WebSocket 事件，应同时检查后端发送、前端分发、store/composable 消费和测试。
...
sequenceDiagram
  ...
  FE->>WS: input:text(text, chat_id, character_id, client_context)
  ...
  WS->>Agent: chat(text, commit_round=false)
  ...
  Agent->>LLM: chat_completion_stream(messages)
  loop LLM chunks
    LLM-->>Agent: chunk
    Agent-->>WS: chunk
...
| speech_start 是打断边界 | 取消当前聊天任务、失效 generation、打断 TTS、通知前端 |
...
WebSocket 成功完成一轮时，路由先向 `ChatStorage` 追加 human 和 ai 消息，再调用 `MemoryManager.on_round_complete()`。这可以让用户可见历史和 LLM 记忆在 generation 维度上保持一致。
...
| client -> server | `input:text` | 文本聊天输入 |
...
2. `frontend/src/utils/websocket.ts` 的事件分发。
...
ASR、VAD、LLM、frontend、routes、translate 等模块后续也应按同样模式补齐长期设计入口。
